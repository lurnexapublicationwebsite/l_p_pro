...
export function generateQuotationRequestPdf(data: QuotationRequestData): Promise<Buffer> {
  // Ensure standard fonts are copied into the bundle folder before PDFKit initializes
  ensureStandardFonts();
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.fontSize(14).font("Helvetica-Bold").text("Requested Books");
      doc.moveDown(0.5);
...
export async function generateClientQuotationPdf(data: ClientQuotationData): Promise<Buffer> {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      // 2. Items Table
      doc.fontSize(11).font("Helvetica-Bold").text("Items Breakdown", 50, doc.y);
      doc.moveDown(0.5);
